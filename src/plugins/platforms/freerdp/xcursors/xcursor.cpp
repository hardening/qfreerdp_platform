...
static XcursorImage *XcursorImageCreate (int width, int height)
{
    ...
    image = (XcursorImage *)malloc(sizeof(XcursorImage) + width * height * sizeof (XcursorPixel));
    if (!image)
    	return NULL;
...
static XcursorImages *XcursorImagesCreate (int size) {
    ...
    images = (XcursorImages *)malloc (sizeof (XcursorImages) + size * sizeof (XcursorImage *));
    if (!images)
    	return NULL;
    images->nimage = 0;
...
static void XcursorImagesSetName (XcursorImages *images, const char *name)
{
    char *newOne;
    ...
    newOne = (char *)malloc(strlen(name) + 1);
    if (!newOne)
    	return;
...
static XcursorFileHeader *_XcursorFileHeaderCreate (int ntoc)
{
    XcursorFileHeader	*fileHeader;

    if (ntoc > 0x10000)
    	return NULL;
    fileHeader = (XcursorFileHeader *)malloc(sizeof (XcursorFileHeader) +
...
static XcursorImages *XcursorXcFileLoadImages(XcursorFile *file, int size)
{
    XcursorFileHeader *fileHeader;
    XcursorDim bestSize;
    int	nsize;
...
static XcursorImages *XcursorFileLoadImages(FILE *file, int size) {
    XcursorFile	f;
...
static const char *XcursorLibraryPath (void)
{
...
static char *_XcursorBuildFullname(const char *dir, const char *subdir, const char *file) {
    ...
    full = (char *)malloc (strlen (dir) + 1 + strlen (subdir) + 1 + strlen (file) + 1);
    if (!full)
    	return NULL;
    full[0] = '\0';
...
static void load_all_cursors_from_dir(const char *path, int size, void (*load_callback)(XcursorImages *, void *), void *user_data)
{
	FILE *f;
...
void xcursor_load_theme(const char *theme, int size, void (*load_callback)(XcursorImages *, void *), void *user_data)
{
	...
	for (path = XcursorLibraryPath(); path; path = _XcursorNextPath(path)) {
		dir = _XcursorBuildThemeDir(path, theme);
		if (!dir)
			continue;
		...
		if (full) {
			load_all_cursors_from_dir(full, size, load_callback, user_data);
			free(full);
		}
